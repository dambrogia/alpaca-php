...
final class OrdersTest extends TestCase
{
    ...
    {
        $response = $this->createV2Client()->orders()->create([
            'symbol' => 'AGCO',
            'qty' => 1,
            'side' => 'buy',
            'type' => 'market',
    ...
    {
        $response = $this->createV2Client()->orders()->get();
    ...
    {
        $response = $this->createV2Client()->orders()->getById($id);
        $data = json_decode($response->getBody()->getContents(), true);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    {
        $response = $this->createV2Client()->orders()->getByClientOrderId($clientOrderId);
        $data = json_decode($response->getBody()->getContents(), true);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    {
        $response = $this->createV2Client()->orders()->delete($id);
